...
async def check_yoomoney_payment_async(label):
    try:
        ...
        logger.warning(f"⏱️ Timeout проверки платежа для {label}")
        return False
    except Exception as e:
        ...
        async for dialog in client.iter_dialogs():
            ...
            async for msg in client.iter_messages(dialog.id, limit=100):
                if not msg or not msg.text or msg.out:
                    continue
                text = msg.text
                if any(word in text.lower() for word in ["login", "new device", "terminate", "незавершенная попытка"]):
                    continue
...
async def show_terms(update_or_query, user_id):
    text = (
        "📋 *ДОБРО ПОЖАЛОВАТЬ!*\n\n"
        "Прежде чем мы начнём, давай познакомимся с правилами 🤝\n\n"
        "✨ *Что важно знать:*\n\n"
        "1️⃣ Аккаунты могут получать временные ограничения от Telegram "
        "(обычно на 24 часа). Это не наша вина, а особенность платформы.\n\n"
        "2️⃣ Мы всегда на связи и поможем, если что-то пойдёт не так.\n\n"
        "3️⃣ Если аккаунт не работает, мы сделаем возврат или заменим.\n\n"
        "4️⃣ Мы работаем честно и ценим каждого клиента! 💙\n\n"
        "✅ Нажимая «СОГЛАСЕН», ты подтверждаешь, что ознакомился с правилами."
    )
    keyboard = kb([btn("✅ СОГЛАСЕН", "agree_terms")])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=kb(
            ...
            [btn("📢 ПОДПИСАТЬСЯ НА КАНАЛ", "sub_channels")]
        )
    )
...
async def sub_channels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "📢 *ПОДПИШИТЕСЬ НА КАНАЛ*\n\n🔹 [Best PONCHI Shop](https://t.me/ponchinoscam)",
        parse_mode="Markdown",
        reply_markup=kb([btn("🔍 ПРОВЕРИТЬ ПОДПИСКУ", "check_sub")], [btn("🔙 НАЗАД", "start")])
    )
...
async def reviews(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("⭐ ОТЗЫВЫ\n\n👉 https://t.me/poncisnop", reply_markup=back("start"))
...
logger.info("=" * 60)
# =====================================================================
# ЧАСТЬ 7: ДОБАВЛЕНИЕ НОМЕРА + МАГАЗИН
# =====================================================================
...
async def add_phone_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if price < 0:
            await update.message.reply_text("❌ Отрицательная цена!")
            return PHONE_PRICE
...
async def add_phone_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        ...
        await update.message.reply_text(
            f"✅ ВОШЁЛ В АККАУНТ!\n\n📱 {phone}\n👤 {me.first_name}\n🆔 `{me.id}`\n📅 Создан: {creation_text}\n⏳ Возраст: {age_text} дней"
        )
        if result:
            await update.message.reply_text(
                f"✅ НОМЕР ДОБАВЛЕН!\n\n📱 {phone}\n💰 {price_rub} ₽\n⭐ {price_stars} Stars\n📅 Возраст: {age} дней"
...
async def delete_phone_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for pid, phone, price_rub, price_stars, age in products:
        rows.append([btn(f"🗑️ {phone} ({age} дн.)", f"del_phone_{pid}")])
...
async def shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for pid, phone, price_rub, price_stars, age in products:
        ...
        rows.append([btn(f"📱 {hidden_phone} - {price_stars}⭐ / {price_rub}₽ ({age} дн.)", f"select_phone_{pid}")])
    rows.append([btn("🔙 НАЗАД", "start")])
    await query.message.reply_text(f"🛒 ВЫБЕРИ НОМЕР:\n\n📱 Доступно: {len(products)}", reply_markup=kb(*rows))
...
logger.info("=" * 60)
logger.info("✅ ЧАСТЬ 7 ЗАГРУЖЕНА: ДОБАВЛЕНИЕ НОМЕРА + МАГАЗИН")
logger.info("=" * 60)
# =====================================================================
...
def main():
    ...
    try:
        ...
        for client in telethon_clients_cache.values():
            try:
