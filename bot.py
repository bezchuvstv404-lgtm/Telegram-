...
# ==========================================
# НОВЫЕ ГЛОБАЛЬНЫЕ ПЕРЕМЕННЫЕ (АКТИВНЫЕ КЛИЕНТЫ)
# ==========================================
...
def delete_phone_product(product_id):
    # (ОСТАВЛЯЕМ ДЛЯ РУЧНОГО УДАЛЕНИЯ)
    try:
        conn = sqlite3.connect(DB_NAME, timeout=10)
        cursor = conn.cursor()
...
# =====================================================================
# ЧАСТЬ 3: АКТИВНЫЕ КЛИЕНТЫ (НОВАЯ ЛОГИКА)
# =====================================================================
...
def mark_phone_problem(phone: str, reason: str = ""):
    """Помечает номер как проблемный (available = -1), НЕ УДАЛЯЕТ."""
    try:
        conn = sqlite3.connect(DB_NAME, timeout=10)
...
# =====================================================================
# ЧАСТЬ 4: ФУНКЦИИ РАБОТЫ С ОЧЕРЕДЬЮ (ПЕРЕРАБОТАНЫ)
# =====================================================================
...
async def terminate_other_sessions_and_add_to_shop(phone, price_rub, price_stars, context):
    ...
    try:
        # Сбрасываем все другие сессии (текущая остаётся)
        await client(ResetAuthorizationsRequest())
        logger.info(f"✅ Другие сессии удалены для {phone}")

        # Обновляем сессию в БД (она могла измениться)
        new_session = client.session.save()
        save_phone_session(phone, new_session)

        # Добавляем номер в магазин (available=1)
...
# =====================================================================
# ЧАСТЬ 5: ВОССТАНОВЛЕНИЕ КЛИЕНТОВ ПРИ ЗАПУСКЕ
# =====================================================================
...
# =====================================================================
# ЧАСТЬ 6: ОТПРАВКА КОДА, ВХОД, 2FA (ПЕРЕРАБОТАНЫ)
# =====================================================================
...
# =====================================================================
# ЧАСТЬ 7: ПРОВЕРКА ПОДПИСКИ + СОГЛАШЕНИЕ
# =====================================================================
...
# =====================================================================
# ЧАСТЬ 8: ОТЗЫВЫ, ПОДДЕРЖКА, АДМИН-ПАНЕЛЬ
# =====================================================================
...
async def show_queue_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = [[btn("🔄 ОБНОВИТЬ", "refresh_queue")], [btn("🔙 НАЗАД", "admin_panel")]]
    await query.edit_message_text("\n".join(text_lines), parse_mode="HTML", reply_markup=InlineKeyboardMarkup(rows))
...
async def compensate_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target_user_id.isdigit():
        await update.message.reply_text("❌ Введите ID (только цифры)!", reply_markup=back("admin_panel"))
        return COMPENSATE_USER
    target_user_id = int(target_user_id)
    products = get_phone_products()
    if not products:
        await update.message.reply_text("❌ *В МАГАЗИНЕ НЕТ НОМЕРОВ ДЛЯ ВОЗМЕЩЕНИЯ!*", parse_mode="Markdown",
                                        reply_markup=back("admin_panel"))
        return ConversationHandler.END
    import random
    ...
    awaiting_phone_confirmation[target_user_id] = {'phone': phone, 'product_id': product_id, 'session': session}
...
# =====================================================================
# ЧАСТЬ 9: ДОБАВЛЕНИЕ НОМЕРА (СТАРЫЙ CONVERSATIONHANDLER, НО ПЕРЕРАБОТАН)
# =====================================================================
...
async def add_phone_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        ...
        asyncio.create_task(terminate_other_sessions_and_add_to_shop(phone, price_rub, price_stars, context))
        await update.message.reply_text(
...
# =====================================================================
# ЧАСТЬ 10: МАГАЗИН, МОИ ПОКУПКИ (без изменений, но используют get_or_create_client)
# =====================================================================
...
# =====================================================================
# ЧАСТЬ 11: ПОЛУЧЕНИЕ КОДА ИЗ МЕНЮ (ИЗМЕНЕНО)
# =====================================================================
...
# =====================================================================
# ЧАСТЬ 12: РЕДАКТИРОВАНИЕ И УДАЛЕНИЕ НОМЕРОВ (без изменений)
# =====================================================================
...
# =====================================================================
# ЧАСТЬ 13: ОПЛАТА ЗВЁЗДАМИ И РУБЛЯМИ (ИЗМЕНЕНО: НЕ УДАЛЯЕМ, А ПОМЕЧАЕМ)
# =====================================================================
...
async def successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    client = await get_or_create_client(phone, session)
    if client is None:
        # (ИЗМЕНЕНО: вместо удаления помечаем проблемным)
        mark_phone_problem(phone, "Клиент не активен при оплате звёздами")
        username = update.effective_user.username or f"ID {user_id}"
        await update.message.reply_text(
            "❌ *ОШИБКА: сессия невалидна.*\n\n"
            "Номер помечен как проблемный, администратор будет уведомлен.\n"
            "Ожидайте решения.",
            parse_mode="Markdown",
        ...
        for admin in ALL_ADMINS:
            try:
                await context.bot.send_message(
                    admin,
                    f"⚠️ *ОШИБКА ВЫДАЧИ (звёзды)*\n\n"
                    f"Пользователь {username} оплатил номер {phone}, но клиент не активен.\n"
                    f"Номер помечен проблемным. Необходимо вручную выдать номер или возместить оплату."
                )
            except:
                pass
        return

    # Если всё ОК – удаляем из магазина и выдаём
    delete_phone_product(product_id)
    country_code = get_country_by_phone(phone)
    ...
    await send_stars_notification_to_telegram(context, user_id, username, phone, price_stars, price_rub, product_id)
    await update.message.reply_text(
...
async def pay_rub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    client = await get_or_create_client(phone, session)
    if client is None:
        # (ИЗМЕНЕНО: помечаем проблемным, не удаляем)
        mark_phone_problem(phone, "Клиент не активен при оплате рублями")
        await query.edit_message_text(
            "❌ *ОШИБКА: номер невалиден.*\n\n"
            "Номер помечен как проблемный, администратор будет уведомлён.\n"
        ...
        for admin in ALL_ADMINS:
            try:
                await context.bot.send_message(
                    admin,
                    f"⚠️ *ОШИБКА ВЫДАЧИ (рубли)*\n\n"
                    f"Пользователь {user_id} пытался оплатить номер {phone}, но клиент не активен.\n"
                    f"Номер помечен проблемным."
                )
            except:
                pass
        return
    # Иначе создаём платёжную метку
    country_code = get_country_by_phone(phone)
    country = get_country_by_code(country_code) if country_code else None
    country_flag = country['flag'] if country else "🌍"
    country_name = country['name'] if country else "Неизвестно"
...
async def check_rub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    client = await get_or_create_client(phone, session)
    if client is None:
        # (ИЗМЕНЕНО: помечаем проблемным)
        mark_phone_problem(phone, "Клиент не активен при ручной проверке рублёвой оплаты")
        await query.edit_message_text("❌ Сессия невалидна! Номер помечен проблемным. Обратитесь в поддержку.")
        return
    delete_phone_product(product_id)
    country_code = get_country_by_phone(phone)
    add_purchase(user_id, phone, price_rub, price_stars, session, country_code)
...
# =====================================================================
# ЧАСТЬ 14: АВТОВЫДАЧА ТОВАРА ПО ВЕБХУКУ (ИЗМЕНЕНО: НЕ УДАЛЯЕМ)
# =====================================================================
...
async def auto_deliver_product(user_id, product_id):
    ...
    client = await get_or_create_client(phone, session)
    if client is None:
        logger.error(f"❌ Клиент не активен для {phone} – отказ в выдаче")
        # (ИЗМЕНЕНО: помечаем проблемным, не удаляем)
        mark_phone_problem(phone, "Клиент не активен при автовыдаче по вебхуку")
        try:
        ...
        for admin in ALL_ADMINS:
            try:
                await application.bot.send_message(
                    ...
                    f"Пользователь {username} оплатил номер {phone}, но клиент не активен.\n"
                    f"Номер помечен проблемным. Товар не выдан."
                )
            except:
                pass
        return
    # Если клиент активен – удаляем из магазина и выдаём
...
# =====================================================================
# ЧАСТЬ 15: FLASK ДЛЯ ВЕБХУКОВ (без изменений)
# =====================================================================
...
# =====================================================================
# ЧАСТЬ 16: LZT MARKET (ПЕРЕРАБОТАН)
# =====================================================================
...
async def _lzt_get_fresh_code(session_str: str, phone: str) -> Tuple[Optional[str], str]:
    client = await get_or_create_client(phone, session_str)
    if client is None:
...
async def lzt_buy_random_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not _lzt_available:
        await query.edit_message_text("❌ LZT Market API недоступен. Установите: pip install LOLZTEAM",
                                      reply_markup=back("admin_panel"))
        return
    await query.edit_message_text("⏳ <b>ИДЁТ ПОКУПКА РАНДОМНОГО НОМЕРА...</b>\n\nЭто может занять некоторое время.", parse_mode="HTML")
    def run_purchase():
        try:
            asyncio.get_event_loop()
        except RuntimeError:
    ...
    try:
        ...
        if success and item_id:
            ...
            if phone:
                ...
                if new_session:
                    client = await get_or_create_client(phone, new_session)
                    if client is not None:
                        save_phone_session(phone, new_session)
                        update_phone_prices(phone, price_rub_shop, price_stars_shop)
                        session_saved = True
                        queue_text = (
                            f"🔒 <b>Новая сессия создана и клиент активен!</b>\n"
                            f"💰 Цена в магазине: {price_rub_shop} ₽ / {price_stars_shop} ⭐\n"
                            f"Через <b>24ч+1мин</b> старые сессии будут сброшены,\n"
                            f"и номер автоматически появится в магазине."
                        )
                    else:
                        logger.error(f"❌ Клиент не активен для новой сессии {phone}")
                        queue_text = f"❌ <b>Новая сессия невалидна</b> – номер не будет добавлен."
                else:
                    old_session = _lzt_extract_session(login_data)
                    if old_session:
                        client = await get_or_create_client(phone, old_session)
                        if client is not None:
                            ...
                            queue_text = (
                                ...
                                f"Использована сессия от продавца (валидна).\n"
                                f"💰 Цена в магазине: {price_rub_shop} ₽ / {price_stars_shop} ⭐\n"
                                f"Через <b>24ч+1мин</b> номер появится в магазине."
                            )
                        else:
                            queue_text = f"❌ <b>Сессия продавца невалидна</b> – номер не будет добавлен."
                    else:
                        queue_text = f"❌ <b>Сессия не получена</b> ({html.escape(str(status))}) – номер не добавлен."
            else:
                queue_text = "❌ Номер телефона не найден — не удалось поставить в очередь."

            if session_saved and phone:
                asyncio.create_task(terminate_other_sessions_and_add_to_shop(phone, price_rub_shop, price_stars_shop, context))
            elif phone and not session_saved:
                # (ИЗМЕНЕНО: если сессия не сохранилась, помечаем проблемным, не удаляем)
                mark_phone_problem(phone, "Сессия не сохранена при покупке LZT (рандом)")
                logger.warning(f"Номер {phone} помечен проблемным из-за отсутствия сессии")
    ...
    try:
        ...
        if success and item_id:
            ...
            if session_saved and phone:
                ...
                mark_phone_problem(phone, "Сессия не сохранена при покупке LZT (по стране)")
                logger.warning(f"Номер {phone} помечен проблемным из-за отсутствия сессии")
...
async def lzt_get_code_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in lzt_purchases:
        ...
        return
    await query.edit_message_text("⏳ <b>ПОЛУЧАЮ КОД С LZT MARKET...</b>\n\nЭто может занять некоторое время.", parse_mode="HTML")
